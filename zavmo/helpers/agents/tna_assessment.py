...
class transfer_to_discover_stage(StrictTool):
    """Transfer to the Discovery stage when the learner has performed TNA Assessment for all NOS competencies."""
    ...
    def execute(self, context: Dict):
        ...
        **TNA Assessment Data:**
        {assessment_details}
        """
        context['skills_assessment'] = assessment_details
        return Result(agent=agent, context=context)

class UpdateCompetency(StrictTool):
    """Use this tool while moving to assess next competency/skill. An evaluation of the learner's assessment in previously assessed competency/skill"""
    ...
    def execute(self, context: Dict):
        ...
        tna_assessment.competency = self.competency
